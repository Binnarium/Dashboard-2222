  ...
  private readonly loadConfigurationSub: Subscription = this.configuration$.subscribe(
    conf => {
      if (conf?.enabledPages) {
        const checkboxFields: Array<keyof CityEnabledPagesDto> = ['activities', 'project', 'questionary', 'clubhouse'];
        checkboxFields.forEach(field => this.enabledPagesControl.controls[field].setValue(conf.enabledPages![field], { emitEvent: false })
        )
      }
      if (conf?.icon)
